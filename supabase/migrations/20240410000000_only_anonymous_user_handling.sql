...
BEGIN
  ...
  IF NEW.raw_user_meta_data->>'is_anonymous' = 'true' THEN
    -- Log this event
    INSERT INTO public.logs (event, user_id, details)
...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'logs') THEN
    ...
    CREATE INDEX idx_logs_event ON public.logs(event);
...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Service role can do everything with logs" ON public.logs
      FOR ALL 
      TO service_role
      USING (true)
      WITH CHECK (true);
